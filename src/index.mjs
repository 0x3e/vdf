function node_steam_vdf(key, val) {
  ...
  for (; i < j; i++) {
    ...
    while (true) {
      ...
      if (val === undefined) {
        ...
        else k_v = node_steam_vdf(key, val)

        stack[stack.length - 1][k_v.k] = k_v.v
      }
      break
    }
  }
...
export function stringify(object) {
  if (typeof object !== "object") {
    throw new TypeError(
      "VDF | Stringify: First input parameter is not an object",
    )
  }
  return create(object)
}

function create(object, level = 0) {
  if (typeof object !== "object") {
    throw new TypeError(
